...
%   ifReg:          whether to use regularization for correlated data
...
%% reduced model without lick

X_reduced = []; %matrix of all the predictors
mask_reduced = [];
for k = 1:numel(eventTime)-2  %for each event type
    predictor = zeros(numel(t),numel(idx));
    ...
    X_reduced = [X_reduced predictor];
    
    %later, we will get a 1-dimensional arrays for the beta for each
    %predictor. We will want to separate the predictors by the event type
    %let's say idx is 3 element long, and there are 2 event types;
    %mask{1} = [1 1 1 0 0 0]; mask{2} = [0 0 0 1 1 1]
    mask_reduced{k} = [false((k-1)*numel(idx),1); true(numel(idx),1); false((numel(eventTime)-2-k)*numel(idx),1)];
end
...
if ifReg
    ...
    %MDl=fitrlinear(X(trainIdx,:)',signal(trainIdx)','ObservationsIn','columns','Learner','leastsquares','Solver','sgd','Lambda',0,'FitBias',true);
    %MDl_reduced=fitrlinear(X_reduced(trainIdx,:)',signal(trainIdx)','ObservationsIn','columns','Learner','leastsquares','Solver','sgd','Lambda',0,'FitBias',true);
    %mdl = fitglm(X(trainIdx,:)',signal(trainIdx)', 'linear');
    MDl=fitrlinear(X',signal','ObservationsIn','columns','Learner','leastsquares','Solver','sgd','Lambda',0,'FitBias',true);
    MDl_reduced=fitrlinear(X_reduced',signal','ObservationsIn','columns','Learner','leastsquares','Solver','sgd','Lambda',0,'FitBias',true);
   
    output.lambda = NaN;
end
%evaluate fit using test set
y_fit=predict(MDl,X);
y_reduced_fit = predict( MDl_reduced,X_reduced);

% amount of variance explained
SST = sum((signal - mean(signal)).^2);
SSE = sum((signal - y_fit).^2);
% % Calculate R^2
R_squared = 1 - (SSE / SST);

SST = sum((signal - mean(signal)).^2);
SSE = sum((signal - y_reduced_fit).^2);
% 
% % Calculate R^2
R_squared_reduced = 1 - (SSE / SST);
% %the GLM coefficients
% ytrain_fit=predict(MDl,X(trainIdx,:));
% SST = sum((signal(trainIdx) - mean(signal(trainIdx))).^2);
% SSE = sum((signal(trainIdx) - ytrain_fit).^2);
% 
% % Calculate R^2
% R_squared = 1 - (SSE / SST);
...
for k = 1:numel(eventTime)  %for each event type
    ...
    output.rSquare = R_squared;
    if k <= numel(eventTime)-2
        output.beta_reduced{k} = MDl_reduced.Beta(mask_reduced{k});
        output.rSquare_reduced = R_squared_reduced;
    end
end
