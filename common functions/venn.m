...
%               d = d12 or [d12 d13 d23]
    ...
    % add text
    if ~isempty(vennOpts.Labels)
        
       for ll = 1:length(vennOpts.Labels)
            hold on;
            text(x(ll),y(ll),vennOpts.Labels{ll})
       end
    end
    ...
    if (nOut==1 && ~vennOpts.Plot) || nOut==2
        if nCirc == 2
            ...
            if ~isempty(vennOpts.Labels)
                ...
                %1,2,3,12,13,23,123
                indArea(1) = A0(1)+I0(4)-I0(1)-I0(2);
                indArea(2) = A0(2)+I0(4)-I0(1)-I0(3);
                indArea(3) = A0(3)+I0(4)-I0(2)-I0(3);
                indArea(4) = I0(1)-I0(4);
                indArea(5) = I0(2)-I0(4);
                indArea(6) = I0(3)-I0(4);
                indArea(7) = I0(4);
                for tt = 1:length(indArea)
                    hold on;
                    text(zoneCentroids(tt,1),zoneCentroids(tt,2), num2str(indArea(tt)));
                end        
            end
        end
    end
...
function [vennOpts, patchOpts] = parsePVPairs (p, v, nZones)
    ...
    vennParamNames = {'plot', 'Labels','ErrMinMode', 'parent'};
    ...
    vennOpts = defaultVennOptions;
    pInd = 1:length(p);
    for pp = 1:length(isVennParam)
        vennOpts.(p{pInd(isVennParam(pp))}) = v{pInd(isVennParam(pp))};
    end
    %PLOT
    i = find(idx==1, 1);
    if i
