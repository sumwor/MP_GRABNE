...
for l=1:nPredictor
    for k=1:1+nback
        ...
        percentage = 100*sum(coeff(:,currPredictor,:)<pvalThresh,3)/nCells;
...
%         end
        ...
        if isempty(input_ctrl) % if no input control regression, use binomial test
            sig=[];
            for ll=1:numel(t)
                [p]=myBinomTest(sum(coeff(ll,currPredictor,:)<pvalThresh,3),nCells,pvalThresh);
                sig(ll)=p;
            end
        ...
        for l = 1:nInteraction
            for k=1:1+nback
                ...
                if isempty(input_ctrl)
                    prop=[]; sig=[];
                    for ll=1:numel(t)
                        prop(ll)=sum(coeff(ll,currPredictor,:)<pvalThresh,3)/nCells; %proportion of cells that is significant
