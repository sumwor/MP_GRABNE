...
clusterMat = pref';
        ...
        % rearrange the groups by the covered area of the first 2 seconds
          cellInd = 1:nCells;
        line11 = nanmean(pref(:,cellInd(T==1)),2);
        line22 = nanmean(pref(:,cellInd(T==2)),2);
        line33 = nanmean(pref(:,cellInd(T==3)),2);
...
% save the data
        ...
        for gg=1:length(clustInd)
            for yy = gg:length(clustInd)
                coeff = corrcoef(input.coeff(clustInd(gg),:),input.coeff(clustInd(yy),:));
