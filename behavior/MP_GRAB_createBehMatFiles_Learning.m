function newDataIndex = MP_GRAB_createBehMatFiles_Learning(dataIndex,Mol)
...
for ii = 1:nFile
    ...
    if isnan(behIndex.BehCreated(ii))
        ...
        save(fullfile(dataIndex.BehPath{ii}, [dataIndex.LogFileName{ii}(1:end-4),'_beh.mat'])',...
    ...
    behIndex.Animal(ii) = {dataIndex.LogFilePath{ii}(Ind(end)+1:end)};
...
% %recordSite = {'left','right','right','left','right','right'};
 ...
 if strcmp(Mol,'NE')
    ...
    recordSite = {'right','right','right','right'};
