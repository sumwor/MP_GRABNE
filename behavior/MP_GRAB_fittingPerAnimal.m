function MP_GRAB_fittingPerAnimal(dataIndex,save_path)
% %MP_fittingPerAnimal %
...
model{5}.initpar = [0.1 0 0.1 0.1]; % initial [alpha beta alpha_K beta_K]
model{5}.lb = [0 0 0 0];
...
for k=1:numel(model)
    ...
    if (runFit)
        for j = 1:numel(animalList)
            ...
            stats_all = MP_GRAB_merge_sessions(dataIndex(currAnimalSessions,:));
